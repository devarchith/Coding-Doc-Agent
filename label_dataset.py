...
def run_one(rec: dict, provider, top_k: int = 5) -> dict:
    ...
    label_counts = dict(Counter(i.label for i in filter_result.items))

    # Best explanation from the first inconsistent item, or first item overall
    explanation = ""
    for item in filter_result.items:
        if item.label != "CONSISTENT":
            explanation = item.explanation
            break
    if not explanation and filter_result.items:
        explanation = filter_result.items[0].explanation

    # Confidence: average across items
    confidences = [i.confidence for i in filter_result.items if hasattr(i, "confidence")]
    confidence  = round(sum(confidences) / len(confidences), 3) if confidences else 0.0

    # Pipeline label (more specific than decision for non-CONSISTENT cases)
    pipeline_label = decision
...
def main() -> None:
    ...
    print(f"Provider: {'MockProvider' if args.mock else 'Gemini gemini-2.0-flash'}")
    print(f"{'='*60}")
